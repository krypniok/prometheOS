...
int dobby(const char* filename)
{
  ...
  /* load the program to execute from HomebrewDB via stdio_fs */
  FILE *fp; int i=0;
  if((fp=fopen(filename, "rb"))==NULL) { printf("file not found: %s\n", filename); return 1; }
  i = (int)fread(p_buf, 1, PROG_SIZE-1, fp);
  if (i < 0) i = 0;
  p_buf[i] = '\0'; // ensure NUL-terminated source buffer for the interpreter
  fclose(fp);
