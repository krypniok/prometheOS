...
static uint16_t g_Shadow[MAX_COLS*MAX_ROWS];
static int g_BatchDepth = 0;
static uint16_t g_Live[ MAX_COLS*MAX_ROWS ];

// Scrollback ring buffer (lines of 80 cells)
static uint16_t *g_SbBuf = 0;       // allocated at init
static int g_SbCap = 0;             // number of lines capacity
static int g_SbHead = 0;            // next write index
static int g_SbCount = 0;           // how many valid lines
static int g_ViewOffset = 0;        // lines above bottom (0 = live)
static size_t g_VgaFontHeight = 16;
...
int setpal() {
    ...
    g_ConsoleX = 0;
    g_ConsoleY = 0;
    // initialize shadow and live buffers with blanks
    for (int i=0;i<MAX_COLS*MAX_ROWS;i++) {
        uint16_t v = (uint16_t)((g_ConsoleColor<<8) | ' ');
        g_Shadow[i] = v; g_Live[i] = v;
    }
    // lazy init scrollback ~1 MiB if not yet
    if (!g_SbBuf) console_scrollback_init(1024*1024);
    return 0;
}

void load_vga_font(const uint8_t *font, size_t height) {
    if (height == 0) height = 16; if (height > 32) height = 32;
    g_VgaFontHeight = height;
...
void save_vga_font(uint8_t *font, size_t height) {
    if (height == 0) height = g_VgaFontHeight; if (height > 32) height = 32;
    port_byte_out(0x3C4, 0x02);
    port_byte_out(0x3C5, 0x04);
    port_byte_out(0x3C4, 0x04);
    port_byte_out(0x3C5, 0x06);
...
size_t vga_get_font_height(void){ return g_VgaFontHeight; }
...
static inline void vga_write_cell(int offset, char ch, unsigned char attr){
    uint8_t *vidmem = (uint8_t *)VIDEO_ADDRESS;
    vidmem[offset] = (uint8_t)ch;
    vidmem[offset+1] = attr;
}

// fwd decl for helper used before definition
static inline void sb_append_row_from_shadow(int row);

static inline void maybe_flush(){ if (g_BatchDepth == 0) console_flush_to_vga(); }

static inline void ensure_scrollback(void){ if (!g_SbBuf) console_scrollback_init(1024*1024); }

void console_begin_batch(void){ g_BatchDepth++; }
void console_end_batch(void){ if (g_BatchDepth > 0) g_BatchDepth--; if (g_BatchDepth == 0) console_flush_to_vga(); }

void set_char_at_video_memory(char character, int offset) {
    ensure_scrollback();
    int cell = offset/2;
    if (cell >= 0 && cell < MAX_COLS*MAX_ROWS) {
        uint16_t v = (uint16_t)((g_ConsoleColor<<8) | ((uint8_t)character));
        // Always update live buffer
        g_Live[cell] = v;
        // If not in scrollback view, update visible shadow too
        if (g_ViewOffset == 0) g_Shadow[cell] = v;
    }
    // Only write-through to VGA if showing live view
    if (g_ViewOffset == 0) vga_write_cell(offset, character, g_ConsoleColor);
}

int scroll_ln(int offset) {
    ensure_scrollback();
    // Before the top line disappears, append it to scrollback
    sb_append_row_from_shadow(0);
    // scroll shadow up by 1 row
    for (int r=0; r<MAX_ROWS-1; r++)
        for (int c=0; c<MAX_COLS; c++)
            g_Shadow[r*MAX_COLS+c] = g_Shadow[(r+1)*MAX_COLS+c];
    // clear last row
    for (int c=0;c<MAX_COLS;c++) g_Shadow[(MAX_ROWS-1)*MAX_COLS+c] = (uint16_t)((g_ConsoleColor<<8) | ' ');
    // flush to VGA
    if (g_ViewOffset == 0) maybe_flush();
...
void print_nl() {
    ...
    // Commit the previous line to scrollback
    int prev_row = get_row_from_offset(get_cursor());
    if (prev_row >= 0 && prev_row < MAX_ROWS) sb_append_row_from_shadow(prev_row);
...
void clear_screen() {
    ...
    uint16_t v = (uint16_t)((g_ConsoleColor<<8) | ' ');
    for (int i = 0; i < screen_size; ++i) { g_Shadow[i] = v; g_Live[i] = v; }
    if (g_ViewOffset == 0) console_flush_to_vga();
...
void printChar(unsigned char x, unsigned char y, unsigned char cl, char c) {
    ...
    // update color temporarily
    unsigned char old = g_ConsoleColor;
    g_ConsoleColor = cl;
...
/* ===== Shadow console helpers ===== */
uint16_t* console_get_shadow(void){ return g_Shadow; }

void console_flush_to_vga(void){
    volatile uint16_t *vram = (uint16_t*)VIDEO_ADDRESS;
    for (int i=0;i<MAX_COLS*MAX_ROWS;i++) vram[i] = g_Shadow[i];
}

/* ===== Scrollback implementation ===== */
static inline void sb_append_line(const uint16_t* row){
    if (!g_SbBuf || !row) return;
    int idx = g_SbHead * MAX_COLS;
    for (int i=0;i<MAX_COLS;i++) g_SbBuf[idx+i] = row[i];
    g_SbHead = (g_SbHead + 1) % g_SbCap;
    if (g_SbCount < g_SbCap) g_SbCount++; else {/* overwrite oldest */}
}

void console_scrollback_init(size_t bytes){
    int lines = (int)(bytes / (MAX_COLS * sizeof(uint16_t)));
    if (lines < 64) lines = 64;
    g_SbBuf = (uint16_t*)malloc(lines * MAX_COLS * sizeof(uint16_t));
    if (g_SbBuf) memset(g_SbBuf, 0, lines * MAX_COLS * sizeof(uint16_t));
    if (!g_SbBuf) { g_SbCap = 0; return; }
    g_SbCap = lines; g_SbHead = 0; g_SbCount = 0; g_ViewOffset = 0;
}

static void render_scrollback_view(void){
    if (!g_SbBuf || g_SbCount == 0 || g_ViewOffset <= 0) return;
    volatile uint16_t *vram = (uint16_t*)VIDEO_ADDRESS;
    int available = g_SbCount;
    int off = g_ViewOffset;
    if (off > available) off = available;
    int start = (g_SbHead - off - MAX_ROWS);
    if (start < -g_SbCap*1000) start = -g_SbCap*1000; // avoid overflow
    // Convert start to positive via modulo
    while (start < 0) start += g_SbCap;
    for (int r=0;r<MAX_ROWS;r++){
        int lineIndex = (start + r) % g_SbCap;
        int sbBase = lineIndex * MAX_COLS;
        for (int c=0;c<MAX_COLS;c++) vram[r*MAX_COLS + c] = g_SbBuf[sbBase + c];
    }
}

void console_page_up(void){
    if (!g_SbBuf) return;
    if (g_SbCount == 0) return;
    if (g_ViewOffset < g_SbCount) g_ViewOffset += 1;
    render_scrollback_view();
}

void console_page_down(void){
    if (!g_SbBuf) return;
    if (g_ViewOffset > 0) g_ViewOffset -= 1; else g_ViewOffset = 0;
    if (g_ViewOffset == 0) {
        // restore live view
        for (int i=0;i<MAX_COLS*MAX_ROWS;i++) g_Shadow[i] = g_Live[i];
...
// Hook points: on newline or scroll, append rows to scrollback
static inline void sb_append_row_from_shadow(int row){
    if (!g_SbBuf) return;
    if (row < 0 || row >= MAX_ROWS) return;
    sb_append_line(&g_Shadow[row*MAX_COLS]);
}

/* ===== VGA palette and glyph helpers (text mode) ===== */
void vga_set_palette_entry(int index, uint8_t r, uint8_t g, uint8_t b){
    if (index < 0 || index > 255) return;
    port_byte_out(0x3C8, (uint8_t)index);
    port_byte_out(0x3C9, r);
    port_byte_out(0x3C9, g);
    port_byte_out(0x3C9, b);
}

void vga_set_glyph(uint8_t ch, const uint8_t *rows, size_t height){
    if (!rows) return;
    if (height == 0) height = g_VgaFontHeight; if (height > 32) height = 32;
    // Map plane 2 to CPU for font writes (like in load_vga_font)
    port_byte_out(0x3C4, 0x02); port_byte_out(0x3C5, 0x04);
    port_byte_out(0x3C4, 0x04); port_byte_out(0x3C5, 0x06);
    port_byte_out(0x3CE, 0x04); port_byte_out(0x3CF, 0x02);
    port_byte_out(0x3CE, 0x05); port_byte_out(0x3CF, 0x00);
    port_byte_out(0x3CE, 0x06); port_byte_out(0x3CF, 0x00);

    uint8_t *plane2 = (uint8_t *)0xA0000;
    size_t i = 0;
    for (; i < height; i++) plane2[ch * 32 + i] = rows[i];
    for (; i < 32; i++)       plane2[ch * 32 + i] = 0x00;
