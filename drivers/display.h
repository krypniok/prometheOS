...
// Current VGA font height (rows per glyph)
size_t vga_get_font_height(void);

// Shadow console API
// Returns pointer to 80x25 text cells (uint16_t char|attr)
uint16_t* console_get_shadow(void);
// Copies shadow buffer to VGA text memory (0xB8000)
void console_flush_to_vga(void);
// Batch drawing: suppresses auto-flush between calls and flushes on end
void console_begin_batch(void);
void console_end_batch(void);

// VGA low-level helpers (text mode only)
